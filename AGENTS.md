...
- Migrations live in `crates/core_lib/src/db/migrations/` as numbered `NNNN_name.sql` / `.down.sql` pairs, embedded via `sqlx::migrate!` and run automatically at startup. Never edit an applied migration — add a new pair.
